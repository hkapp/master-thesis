...
%Laurent's plan:
%\begin{itemize}
%\item programming language / compilation strategy in database (high
%level overview of what exists and how their are implemented: interpreted
%only, JITed, ahead-of-time compile etc... See MS SQLServer's T-SQL, Postgres, etc. Maybe touch on the use of llvm for query execution.
...
END;
	...
	\caption{Example of a simple PL/SQL program}
...
SELECT pretty_print(salary) FROM emp;
	...
	\caption{Example of a simple PL/SQL program, and how it can be used in a query as a UDF. The '\sql{||}' is the concatenation operator.}
